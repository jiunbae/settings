...
**Resolved as of 2026-08-05.** `linear-issue.sh update <ID> --labels "<LABEL-1>,<LABEL-2>" --execute` now merges labels correctly, including names that contain commas. Use the helper first.
...
curl -s -X POST https://api.linear.app/graphql -H "Authorization: $LINEAR_API_KEY" \
  ...
  -d '{"query":"query($t:String!){team(id:$t){labels(first:250){nodes{id name parent{name}}}}}","variables":{"t":"<TEAM_ID>"}}'
```
